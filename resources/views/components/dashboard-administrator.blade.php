<x-app-layout>
    ...
    <x-slot name="header_mobile">
		...
		<a href="{{ route('dashboard') }}">
			<img alt="Logo" src="https://preview.keenthemes.com/keen/theme/demo6/dist/assets/media/logos/logo-5.svg"
				class="max-h-30px" />
		</a>
		<!--end::Logo-->
		...
		<div class="d-flex align-items-center">
			<div class="btn btn-icon w-auto btn-clean d-inline-flex align-items-center btn-lg px-2 mr-5">
				<span class="text-muted font-weight-bold font-size-base mr-1">{{ __('dashboard.hi') }}, </span>
				<span class="text-dark-50 font-weight-bolder font-size-base mr-3">{{ Auth::user()->name }}</span>
			...
			<button class="btn btn-icon btn-hover-transparent-white btn-clean btn-lg ml-2" id="kt_header_mobile_topbar_toggle">
				<span class="svg-icon svg-icon-primary svg-icon-2x">
					<!--begin::Svg Icon | path:/var/www/preview.keenthemes.com/keen/releases/2021-04-21-040700/theme/demo6/dist/../src/media/svg/icons/Text/Toggle-Left.svg-->
	...
	<x-slot name="local_link"> {{ $local_link }} </x-slot>

	<x-slot name="local_script"> {{ $local_script }} </x-slot>
						...
						<div class="container-fluid">
							...
							<div class="header-menu-wrapper header-menu-wrapper-left" id="kt_header_menu_wrapper">
								...
								<div class="header-logo mr-10 d-none d-lg-flex">
									<a href="{{ route('dashboard') }}">
										<img alt="Logo"
											src="https://preview.keenthemes.com/keen/theme/demo6/dist/assets/media/logos/logo-5.svg"
											class="h-40px" />
									</a>
								</div>
								<!--end::Logo-->
							...
							<div class="topbar">
								...
								<div class="topbar-item">
									<div
										...
										<span
											...
											>
											<!--begin::Svg Icon | path:/var/www/preview.keenthemes.com/keen/releases/2021-04-21-040700/theme/demo6/dist/../src/media/svg/icons/Communication/Mail-notification.svg-->
											<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
												<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
													<rect x="0" y="0" width="24" height="24"/>
								...
								<div class="topbar-item mr-3">
									<div class="btn btn-icon w-auto btn-clean d-inline-flex align-items-center btn-lg px-2" id="kt_quick_user_toggle">
										<span class="text-muted font-weight-bold font-size-base mr-1">{{ __('dashboard.hi') }}, </span>
										<span class="text-dark-50 font-weight-bolder font-size-base mr-3">{{ Auth::user()->name }}</span>
										<span class="symbol symbol-35 symbol-light-info">
											<span class="symbol-label font-size-h5 font-weight-bold">{{  Str::upper(Str::substr(Auth::user()->name, 0, 1)) }}</span>
										</span>
									</div>
								</div>
								<!--end::User-->
					...
					<div class="footer bg-white py-4 d-flex flex-lg-column" id="kt_footer">
						...
						<div class="container d-flex flex-column flex-md-row align-items-center justify-content-between">
							...
							<div class="nav nav-dark order-1 order-md-2">
								<a href="http://keenthemes.com/keen" target="_blank" class="nav-link pr-3 pl-0">{{ __('dashboard.about') }}</a>
								<a href="http://keenthemes.com/keen" target="_blank" class="nav-link px-3">{{ __('dashboard.team') }}</a>
								<a href="http://keenthemes.com/keen" target="_blank" class="nav-link pl-3 pr-0">{{ __('dashboard.contact') }}</a>
							</div>
							<!--end::Nav-->
						</div>
						<!--end::Container-->
	...
	<!-- begin::User Panel-->
	<div id="kt_quick_user" class="offcanvas offcanvas-right pt-5 pb-10 pl-10 pr-5">
		<!--begin::Header-->
		<div class="offcanvas-header d-flex align-items-center justify-content-between pb-5">
			<h3 class="font-weight-bold m-0">
				{{ __('dashboard.user_profile') }}
			</h3>
			<a href="#" class="btn btn-light btn-text-danger btn-hover-text-danger font-weight-bold mr-2" id="kt_quick_user_close">
				{{ __('dashboard.close') }}
			</a>
		</div>
		<!--end::Header-->
		...
		<div class="offcanvas-content pr-5 mr-n5">
			...
			<div class="d-flex align-items-center mt-5">
                <span class="symbol symbol-80 symbol-light-primary pulse pulse-primary mr-5">
					<span class="symbol-label font-size-h1 font-weight-bolder">{{  Str::upper(substr(Auth::user()->name, 0, 2)) }}</span>
				</span>

				<div class="d-flex flex-column">
					<a href="{{ route('account.index') }}" class="font-weight-bold font-size-h5 text-dark-75 text-hover-primary">{{  Str::ucfirst(Auth::user()->name) }}</a>
					<div class="text-muted mt-1">{{ Str::ucfirst(Str::lower(Auth::user()->profile)) }}</div>
					<div class="navi mt-1">
						<a href="{{ route('account.index') }}" class="navi-item">
							<span class="navi-link p-0 pb-2">
								...
								<span class="navi-text text-muted text-hover-primary" title="{{ Str::lower(Auth::user()->email) }}">{{ Str::substr(Str::lower(Auth::user()->email), 0, 15) }}...</span>
							</span>
						</a>
					</div>
				</div>
					...
					<form method="POST" action="{{ route('logout') }}">
						...
						<span class="navi-link">
							<a
								href="{{ route('logout') }}"
								onclick="
                                    if (confirm('{{ __('validation.are-you-sure-continue') }}')) {
                                ...
                                "
								class="btn btn-sm btn-light-danger font-weight-bold py-3 px-6"
								>
	...
	<div id="kt_quick_panel" class="offcanvas offcanvas-right pt-5 pb-10">
		<!--begin::Header-->
		<div class="offcanvas-header d-flex align-items-center justify-content-between pl-10 pr-5 pb-10">
			<h3 class="font-weight-bold m-0">
				{{ __('dashboard.notifications') }}
			</h3>

			<a href="#" class="btn btn-light btn-text-danger btn-hover-text-danger font-weight-bold mr-2" id="kt_quick_panel_close">
				{{ __('dashboard.close') }}
			</a>
		</div>
		<!--end::Header-->
								...
								<div class="timeline-item">
									...
									<div class="timeline-media bg-light-warning">
										<span class="svg-icon svg-icon-warning svg-icon-md">
											<!--begin::Svg Icon | path:/keen/theme/demo6/dist/assets/media/svg/icons/General/Attachment2.svg-->
											<svg xmlns="http://www.w3.org/2000/svg"
												xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px"
												viewBox="0 0 24 24" version="1.1">
												<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
